...
mod binary {
	...
	pub fn main() {
		...
		let (firmware, expected_target) = if cfg!(feature = "uefi_bin") {
			...
			panic!(
				"Either the `uefi_bin` or `bios_bin` feature must be enabled when the `binary` feature is \
				 enabled"
			);
		};
		if Path::new(&target).file_stem().expect("target has no file stem") != expected_target {
			panic!(
			...
			Err(_) => {
				eprintln!(
					"The KERNEL environment variable must be set for building the bootloader.\n\nPlease use \
					 the `cargo builder` command for building."
				);
				process::exit(1);
			},
		});
		...
		assert!(kernel.exists(), "KERNEL does not exist: {}", kernel.display());
		...
		let second_line =
			second_line_opt.expect(&format!("unexpected llvm-size line output:\n{}", output_str));
		...
		if cfg!(feature = "uefi_bin") {
			...
			file.write_all(
				format!(
					"const KERNEL_SIZE: usize = {}; const KERNEL_BYTES: [u8; KERNEL_SIZE] = \
					 *include_bytes!(r\"{}\");",
					kernel_size,
		...
		if cfg!(feature = "bios_bin") {
			...
			let stripped_kernel_file_name_replaced =
				stripped_kernel_file_name.replace('-', "_").replace('.', "_");
		...
		let config_stream = match env::var("KERNEL_MANIFEST") {
			Err(env::VarError::NotPresent) => {
				panic!(
					"The KERNEL_MANIFEST environment variable must be set for building the \
			...
			Ok(path) if Path::new(&path).file_name().and_then(|s| s.to_str()) != Some("Cargo.toml") => {
				let err = format!(
					"The given `--kernel-manifest` path `{}` does not point to a `Cargo.toml`",
					path,
				);
				quote! { compile_error!(#err) }
			},
			Ok(path) if !Path::new(&path).exists() => {
				let err = format!("The given `--kernel-manifest` path `{}` does not exist.", path);
				quote! {
					compile_error!(#err)
				}
			},
			Ok(path) => {
				...
				let contents = fs::read_to_string(&path)
					.expect(&format!("failed to read kernel manifest file (path: {})", path));
					...
					.is_some()
				{
					...
					let err = format!(
						"no bootloader dependency in {}\n\n  The `--kernel-manifest` path should point to \
						 the `Cargo.toml` of the kernel.",
						path
					);
					quote! {
						compile_error!(#err)
	...
	/// module itself, since cargo currently unifies dependencies (the `toml` crate
	/// enables serde's standard feature). Also, it allows to separate the parsing special
	/// cases such as `AlignedAddress` more cleanly.
	...
	impl quote::ToTokens for AlignedAddress {
		fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) { self.0.to_tokens(tokens); }
	}
	...
	impl serde::de::Visitor<'_> for AlignedAddressVisitor {
		...
		fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
			write!(
				formatter,
				"a page-aligned memory address, either as integer or as decimal or hexadecimal string (e.g. \
				 \"0xffff0000\"); large addresses must be given as string because TOML does not support \
