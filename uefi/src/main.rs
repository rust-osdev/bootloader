...
use uefi::{
    CStr8, CStr16, boot,
    boot::{AllocateType, MemoryType, OpenProtocolAttributes, OpenProtocolParams, SearchType},
    cstr8, cstr16,
...
fn open_pxe_base_code() -> Option<boot::ScopedProtocol<BaseCode>> {
    let handles = boot::locate_handle_buffer(SearchType::from_proto::<BaseCode>()).ok()?;
    for handle in handles.iter().copied() {
        let base_code = unsafe {
            boot::open_protocol::<BaseCode>(
                OpenProtocolParams {
                    handle,
        ...
        if let Ok(base_code) = base_code
            && base_code.mode().dhcp_ack_received()
        {
            return Some(base_code);
        }
    }
    None
}
