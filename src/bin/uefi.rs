...
fn efi_main(image: Handle, st: SystemTable<Boot>) -> Status {
    ...
    let mmap_storage = {
        let max_mmap_size = st.boot_services().memory_map_size() + 8 * mem::size_of::<MemoryDescriptor>();
        let ptr = st
            .boot_services()
    ...
    let system_info = SystemInfo {
        ...
        rsdp_addr: {
            ...
            let rsdp =
                acpi2_rsdp.or_else(|| config_entries.find(|entry| matches!(entry.guid, cfg::ACPI_GUID)));
            rsdp.map(|entry| PhysAddr::new(entry.address as u64))
        },
    };

    bootloader::binary::load_and_switch_to_kernel(&KERNEL.0, frame_allocator, page_tables, system_info);
}

/// Creates page table abstraction types for both the bootloader and kernel page tables.
fn create_page_tables(frame_allocator: &mut impl FrameAllocator<Size4KiB>) -> bootloader::binary::PageTables {
    // UEFI identity-maps all memory, so the offset between physical and virtual addresses is
    // 0
    ...
    let bootloader_page_table = {
        ...
        let new_table: &mut PageTable = {
            let ptr: *mut PageTable = (phys_offset + new_frame.start_address().as_u64()).as_mut_ptr();
        ...
        unsafe {
            x86_64::registers::control::Cr3::write(new_frame, x86_64::registers::control::Cr3Flags::empty());
    ...
    let (kernel_page_table, kernel_level_4_frame) = {
        ...
        (unsafe { OffsetPageTable::new(level_4_table, phys_offset) }, frame)
    };
...
fn panic(info: &PanicInfo) -> ! {
    unsafe { bootloader::binary::logger::LOGGER.get().map(|l| l.force_unlock()) };
    log::error!("{}", info);
    loop {
        unsafe { asm!("cli; hlt") };
    }
