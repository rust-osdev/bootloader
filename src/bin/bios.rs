...
use x86_64::structures::paging::{Mapper, PageTable, PageTableFlags, PhysFrame, Size2MiB, Size4KiB};
...
fn bootloader_main(
    ...
    use bootloader::binary::{
        bios::memory_descriptor::E820MemoryRegion,
    ...
    {
        let start_frame: PhysFrame<Size2MiB> = PhysFrame::containing_address(PhysAddr::new(4096 * 512 * 512));
    ...
    bootloader::binary::load_and_switch_to_kernel(kernel, frame_allocator, page_tables, system_info);
}
...
fn create_page_tables(frame_allocator: &mut impl FrameAllocator<Size4KiB>) -> bootloader::binary::PageTables {
    // We identity-mapped all memory, so the offset between physical and virtual addresses is
    // 0
    let phys_offset = VirtAddr::new(0);
