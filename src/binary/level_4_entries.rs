...
impl UsedLevel4Entries {
	...
	pub fn new<'a>(segments: impl Iterator<Item = ProgramHeader<'a>>, virtual_address_offset: u64) -> Self {
		...
		for segment in segments {
			let start_page: Page =
				Page::containing_address(VirtAddr::new(segment.virtual_addr() + virtual_address_offset));
	...
	///
	/// This is a convenience method around [`get_free_entry`], so all of its docs applies
	/// here too.
	pub fn get_free_address(&mut self) -> VirtAddr {
		Page::from_page_table_indices_1gib(self.get_free_entry(), PageTableIndex::new(0)).start_address()
	}
}
