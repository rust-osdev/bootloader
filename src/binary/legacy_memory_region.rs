	...
	I::Item: LegacyMemoryRegion,
{
	...
	/// identity-mapping the frame at address zero is not valid in Rust, because Rust's
	/// `core` library assumes that references can never point to virtual address `0`.
	...
	/// Creates a new frame allocator based on the given legacy memory regions. Skips any
	/// frames before the given `frame`.
	...
	pub fn max_phys_addr(&self) -> PhysAddr {
		self.original.clone().map(|r| r.start() + r.len()).max().unwrap()
	}

	/// Converts this type to a boot info memory map.
	///
	/// The memory map is placed in the given `regions` slice. The length of the given
	/// slice must be at least the value returned by [`len`] pluse 1.
	///
	/// The return slice is a subslice of `regions`, shortened to the actual number of
	/// regions.
	...
	fn add_region(
		...
		unsafe { regions.get_mut(*next_index).ok_or(())?.as_mut_ptr().write(region) };
		*next_index += 1;
		Ok(())
	}
}
