...
{
    fn new(bytes: &'a [u8], page_table: &'a mut M, frame_allocator: &'a mut F) -> Result<Self, &'static str> {
    ...
    fn used_level_4_entries(&self) -> UsedLevel4Entries {
        UsedLevel4Entries::new(self.elf_file.program_iter(), self.inner.virtual_address_offset)
    }
}
    ...
    F: FrameAllocator<Size4KiB>,
{
    ...
    /// contain the elf file. Thus writing to memory in that state will cause aliasing
    /// issues. To avoid that, we allocate a new frame, copy all bytes from the old frame
    /// to the new frame, and remap the page to the new frame. At this point the page no
    /// longer aliases the elf file and we can write to it.
    ///
    /// When we map the new frame we also set [`COPIED`] flag in the page table flags, so
    /// that we can detect if the frame has already been copied when we try to modify the
    /// page again.
    ///
    /// ## Safety
    ...
    unsafe fn make_mut(&mut self, page: Page) -> PhysFrame {
        ...
        // Allocate a new frame and copy the memory, utilizing that both frames are identity
        // mapped.
    ...
    fn handle_dynamic_segment(
        ...
        for rela in relas {
            ...
            assert_eq!(idx, 0, "relocations using the symbol table are not supported");
