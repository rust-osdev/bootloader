...
/// your kernel. The naming convention for all config fields is `kebab-case`, otherwise
/// the config keys correspond to the field names of this struct (i.e. just replace `_`
/// with `-`). Unknown config keys lead to an error.
///
/// ## Example
///
/// To map the complete physical memory starting at virtual address
/// `0x0000_4000_0000_0000`, add the following to your kernel's `Cargo.toml`:
///
/// ```toml
...
/// ## Memory Addresses
///
/// Memory addresses must be positive and page aligned. Since TOML does not support
/// unsigned 64-bit integers, we also support string input to specify addresses larger
/// than `i64::MAX`. For example:
///
/// ```toml
/// physical-memory-offset = "0xf000_0000_0000_0000"
/// ```
///
/// The above example would fail if the address was specified as integer instead (i.e.
/// without the quotes).
///
/// All memory addresses are optional, even if their corresponding switch is enabled. If
/// no address is specified, the bootloader will choose an unused entry of the level 4
/// page table at runtime.
#[derive(Debug)]
pub struct Config {
    ...
    ///
    /// If not given, the bootloader searches for a free virtual memory region
    /// dynamically.
    ///
