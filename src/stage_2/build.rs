...
fn main() {
    ...
    build_subproject(Path::new("../bootsector"), &["_start", "real_mode_println", "no_int13h_extensions", "dap_load_failed"], "x86_64-real_mode.json", &out_dir, &objcopy);
}
...
fn build_subproject(dir: &Path, global_symbols: &[&str], target: &str, out_dir: &str, objcopy: &Path) {
    ...
    cmd.arg(format!("--target={}", dir.join("../..").join(target).display()));
